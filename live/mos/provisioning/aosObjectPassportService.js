...
/*
 * Install verbatim to:
 * /var/www/ix-core/mos/provisioning/aosObjectPassportService.js
 */
...
function assertPassportEntity({
   ...
   * They are not silently rewritten here.
   */
  if (
    actualEntityId &&
    ...
    throw new MosError(
      ...
      "AOS Passport belongs to a different Entity.",
      {
        ...
        passportId: cleanText(passport?.passportId),
        expectedEntityId,
        actualEntityId
      },
      409
...
function ensurePassportForAosObject({
  ...
  const normalizedObjectId = requireText(
    objectId,
    "AOS_PASSPORT_OBJECT_REQUIRED",
    "AOS Passport provisioning requires objectId."
  );

  const normalizedEntityId = requireText(
    entityId,
    "AOS_PASSPORT_ENTITY_REQUIRED",
    "AOS Passport provisioning requires entityId."
  );

  const existing = findPassportBySource(
  ...
  if (
    ...
    throw new MosError(
      ...
      "New AOS Passport did not persist the Object Entity identity.",
      {
        ...
        actualEntityId:
          cleanText(passport.entityId) || null
      },
      500
    );
  }
...
function verifyAosObjectPassport({
  ...
  const normalizedPassportId = requireText(
    passportId,
    "AOS_PASSPORT_ID_REQUIRED",
  ...
  const bySource = findPassportBySource(
    AOS_PASSPORT_SOURCE_TYPE,
    normalizedObjectId
  );

  const byId = findPassportById(
  ...
  if (
    !bySource ||
    !byId ||
    bySource.passportId !== normalizedPassportId ||
    byId.passportId !== normalizedPassportId ||
    byId.sourceType !== AOS_PASSPORT_SOURCE_TYPE ||
    cleanText(byId.sourceId) !== normalizedObjectId
  ) {
    throw new MosError(
